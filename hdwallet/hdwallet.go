...
// Wallet is the underlying ethwallet struct.
type Wallet struct {
	mnemonic    string
...
// NewFromMnemonic returns a new ethwallet from a BIP-39 mnemonic.
...
// NewFromSeed returns a new ethwallet from a BIP-39 seed.
...
// the ethwallet is on, however this does nothing since this is not a hardware device.
func (w *Wallet) URL() accounts.URL {
	return w.url
}

// Status implements accounts.Wallet, returning a custom status message
// from the underlying vendor-specific hardware ethwallet implementation,
...
// the ethwallet. If self-derivation was enabled, the account list is
...
// or is not pinned into this ethwallet instance.
...
// explicitly pin to the ethwallet manually. To avoid chain head monitoring, self
...
// SignText requests the ethwallet to sign the hash of a given piece of data, prefixed
